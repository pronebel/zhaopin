	...
	function checkCookies(){
		...
		function _play(){			
			...
			$(".title1").css({'color':'rgba(255,255,255,0.4)'});
			$('#register').css({
				'display':'block'
			})
		...
		function _play2(){			
			...
			$(".title2").css({'color':'rgba(255,255,255,0.4)'});
	...
	/*		var V=$(this).val();
			if(V){
				$(this).next().fadeIn(500).animate({
					'right':'55px'
				},200)
				$(this).nextAll("#judge_false").fadeOut();
			}else{
		...
		function move(){
			...
			if(flag4==2){//register
				$.post("register",
				{id:$("#user").val(),password:$("#password1").val(),data_cookies:'true'},
				function(d){
			//		alert(d);
					if(d){
			...
			//	alert(1);
				$.post("login",
				{id:$("#login .username").val(),password:$("#login .password").val(),data_cookies:'true'},
				function(d){
					if(d){
					//	alert(d);
		...
		$("#register #user").blur(function(){//焦点从id框移除时，发送ajax请求判断id是否可用
			if($(this).val().length<6){
				...
				$.post("registerJudge",{id:$("#user").val()},function(d){
					if(d){
						flag2=1;
		...
		$("#login .username").blur(function(){//login的id框验证
			if($(this).val().length<6){
				...
				ID=$(this).val();
				$.post("loginJudge",{id:ID},function(d){
			      //alert(d);
					if(d){//账号正确
				...
				ID=$(this).val();
				$.post("loginJudge",{id:ID,password:$("#login .password").val()},function(d){
					if(d){//账号密码对应正确
						flag1=1;
						_true();
					}else{
		...
		$("#login .password").blur(function(){
			if($(this).val().length<6){
				...
				$.post("loginJudge",{id:ID,password:$(this).val()},function(d){
					if(d){//账号密码对应正确
						flag1=1;
						_true();
					}else{
